  ...
  switch (context.type) {
    case "maintenance":
      return context.maintenanceRequestId
        ? {
            label: "View Request",
            href: `/owner-dashboard/maintenance?open=${context.maintenanceRequestId}`,
          }
        : context.propertyId
          ? {
              label: "View Maintenance",
              href: `/owner-dashboard/properties/${context.propertyId}?tab=maintenance`,
            }
          : null;
    case "application":
      return context.applicationId
        ? {
            ...
            href: `/owner-dashboard/applications?open=${context.applicationId}`,
          }
        : context.propertyId
          ? {
              label: "View Applications",
              href: `/owner-dashboard/applications?propertyId=${context.propertyId}`,
            }
          : null;
    case "rental":
      return context.rentalId
        ? {
            label: "View Rental",
            href: `/owner-dashboard/rentals?open=${context.rentalId}`,
          }
        : null;
    case "rental-setup":
      return context.propertyId
        ? {
            ...
            href: `/owner-dashboard/properties/${context.propertyId}?tab=rental`,
          }
        : null;
    case "property":
      return context.propertyId
        ? {
            label: "View Property",
            href: `/owner-dashboard/properties/${context.propertyId}`,
          }
        : null;
    default:
      return null;
...
function contextSubtitle(context: ConversationContext | undefined): string {
  ...
  if (context.propertyId)
    return `${propertyTitle(context.propertyId)} · ${context.label}`;
...
function OwnerMessagesPageInner() {
  ...
  const conversations = emptyMode
    ? []
    : getConversationsFor(OWNER_PARTICIPANT_ID);

  const legacyMatch = useMemo(() => {
    if (!legacyContextParam) return undefined;
    return conversations.find((c) => {
      const other = getParticipant(
        c.participantIds.find((id) => id !== OWNER_PARTICIPANT_ID) ?? "",
      );
      return (
        other?.name.toLowerCase().includes(legacyContextParam) ||
        contextSubtitle(c.context).toLowerCase().includes(legacyContextParam)
      );
    });
  ...
  const initialId =
    (openParam && conversations.some((c) => c.id === openParam)
      ? openParam
      : undefined) ??
    legacyMatch?.id ??
    null;
  ...
  const activeOther = activeChat
    ? getParticipant(
        activeChat.participantIds.find((id) => id !== OWNER_PARTICIPANT_ID) ??
          "",
      )
    : undefined;
  ...
  const searchedConversations = useMemo(() => {
    ...
    return conversations.filter((c) => {
      ...
      return (
        ...
        contextSubtitle(c.context).toLowerCase().includes(query)
      );
    });
  ...
  function hasUnread(conversation: Conversation) {
    ...
    return conversation.messages.some(
      (m) => m.senderId !== OWNER_PARTICIPANT_ID && m.ts > lastRead,
    );
  }
              ...
              <div className="flex-1 overflow-y-auto p-2.5">
                {conversations.length === 0 ? (
                  <div className="flex flex-col items-center px-6 py-16 text-center">
                    ...
                    />
                    <h3 className="font-bricolage mt-5 text-lg font-bold text-black">
                      No messages yet
                    </h3>
                    <p className="mt-2 text-sm leading-relaxed text-black/50">
                      Your conversations with renters and property professionals
                      will appear here.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col items-center px-6 py-16 text-center">
                    {searchQuery.trim() ? (
                      <Search
                        aria-hidden="true"
                        className="mb-3 size-8 text-black/20"
                      />
                    ) : null}
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center justify-between gap-1">
                              <h3
                                className={`flex min-w-0 items-center gap-1 text-sm font-semibold ${unread ? "text-black" : "text-neutral-900"}`}
                              >
                              ...
                              <span className="shrink-0 text-[9px] font-medium text-neutral-400">
                                {last ? relativeLabel(last.ts) : ""}
                              </span>
                            </div>
                            <p className="mt-0.5 truncate text-[10px] font-medium text-neutral-500">
                              {contextSubtitle(convo.context) || other.role}
                            </p>
                            <div className="mt-2 flex items-center justify-between gap-2">
                              <p
                                className={`flex-1 truncate text-xs ${unread ? "text-black" : "text-neutral-600"}`}
                              >
                    ...
                    <>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="truncate text-xs text-black/50">
                          {activeOther.role}
                          {activeChat.context?.propertyId
                            ? ` · ${propertyTitle(activeChat.context.propertyId)}`
                            : ""}
                        </p>
                      </div>
                      <button
                ...
                threadQuery && threadMessages.length === 0 ? (
                  <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center">
                    ...
                    <p className="text-sm text-black/50">
                      Nothing matches &quot;{threadQuery}&quot; in this
                      conversation.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center">
                    ...
                    <p className="text-sm text-black/50">
                      Send {activeOther.name.split(" ")[0]} a message below.
                    </p>
                  </div>
                ) : (
                    ...
                    {threadRenderItems.map((item, index) => {
                      ...
                      return (
                        <div key={index}>
                          ...
                          <div
                            className={`flex py-0.5 ${isOwner ? "justify-end" : "justify-start"}`}
                          >
                            <div
                            ...
                            >
                              ...
                              <span
                                className={`float-right mt-1 ml-2 text-[10px] font-medium ${isOwner ? "text-white/60" : "text-neutral-400"}`}
                              >
                                {timeLabel(item.ts)}
                              </span>
                            </div>
