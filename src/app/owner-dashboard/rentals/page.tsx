...
function OwnerRentalsPageInner() {
  ...
  const rentalsAll = emptyMode ? [] : getOwnerRentals();
  const openParam = searchParams.get("open");
  const openRental = openParam
    ? rentalsAll.find((r) => r.id === openParam)
    : null;
        ...
        <div className="mx-auto max-w-[1360px]">
          <header className="border-b border-black/10 pb-8">
            ...
            <p className="text-carbon-600 mt-5 max-w-2xl text-base leading-7 sm:text-lg">
              Every active relationship between one of your properties and a
              renter.
            </p>
          </header>
            ...
            {TABS.map((item) => (
              ...
              >
                ...
                <span className="ml-1 opacity-60">
                  {rentalsAll.filter((r) => r.status === item).length}
                </span>
              </button>
            ))}
          </div>
          ...
          {rentals.length === 0 ? (
            <div className="mt-6 flex flex-col items-center justify-center rounded-[1.5rem] border border-dashed border-black/15 bg-white px-6 py-16 text-center">
              ...
              <h3 className="font-bricolage mt-5 text-xl font-medium">
                No {tab.toLowerCase()} rentals
              </h3>
              <p className="text-carbon-500 mt-2 max-w-sm text-sm leading-6">
                You don&apos;t have any {tab.toLowerCase()} rentals yet.
              </p>
            </div>
          ) : (
                  ...
                  >
                    <div className="relative flex items-center gap-4 p-5">
                      ...
                      <div className="min-w-0">
                        <p className="font-bricolage truncate font-medium">
                          {property.title}
                        </p>
                    ...
                    <dl className="text-carbon-500 grid grid-cols-2 gap-3 border-t border-black/8 px-5 py-4 text-xs">
                      <div>
                        ...
                        <dd className="text-carbon-900 mt-0.5 font-medium">
                          {rental.rent}
                        </dd>
                      </div>
                      <div>
...
function RentalDetail({
  ...
  const maintenance = getMaintenanceRequests().filter(
    (m) => m.propertyId === rental.propertyId,
  );
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-black/10 pb-5">
          <div>
            ...
            <h2 className="font-bricolage mt-1 text-2xl font-medium">
              {property?.title}
            </h2>
        ...
        <dl className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <dd className="mt-1 font-medium">
              {property?.propertyManager?.name ?? "You"}
            </dd>
          </div>
          <div>
        ...
        <div className="mt-6 flex flex-wrap gap-2 border-t border-black/10 pt-5">
          {(() => {
            ...
            const target = property?.propertyManager
              ? {
                  id: property.propertyManager.professionalId,
